...
const BooksPageComponent = () => {
	...
	const addBooks = (book: BooksType) => {
		const existingProduct = cart.books.find(p => p._id === book._id);
		if (existingProduct) {
			toast({ title: 'Books already in cart', position: 'bottom-right', status: 'warning' });
			return;
		}
		addToCart(book);
		toast({ title: 'Books added successfully', position: 'bottom-right' });
	};
